...
const NuevoTurnoPage = () => {
  ...
  const [turno, setTurno] = useState({
    ...
    celular: '',
    email: '',
    cobertura: '', // Para compatibilidad con código anterior
    coberturaMedicaId: '', // Nueva relación con la tabla CoberturaMedica
    servicio: '',
    doctor: '',
    tipoDeTurnoId: '', // Nuevo campo para la relación con TipoTurnoDoctor
    desde: new Date().toISOString().slice(0, 16), // Formato YYYY-MM-DDTHH:mm
    duracion: 30,
    observaciones: ''
  });
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        // NUEVO: Procesar parámetros de URL para autocompletar datos del turno
        ...
        // Verificar si hay parámetros de búsqueda en la URL para paciente
        ...
        if (dniParam) {
          ...
          await buscarPacientePorParam(`/${idParam}`);	
        }
